...
minisign の署名には、署名した時刻が入ります。同じアセットを署名し直せば署名は別物になり、詳細ファイルも
`detail_sum` も動きます。`detail_sum` は、利用者の Amenbo が「このプラグインは新しいビルドになった」と
判断する唯一の材料です。publish のたびに全部を署名し直すカタログは、**何かを1つ載せるたびに、
全プラグインに更新が出たと全利用者へ言う**ことになります。

だから **バイト列が変わっていないなら、署名を持ち越します**。署名する前に、いま自分が publish している
そのプラグインの詳細ファイルを読み、アセットのダイジェストが同じなら、そこに載っている署名をそのまま使います。
持ち越す署名は、いまダウンロードしたバイト列に対して自分の公開鍵で検証してから使ってください——持ち越しは
そのバイト列についての主張であり、主張は確かめる価値があります。読めない・アセットが変わった・検証が通らない
——どれも署名し直すだけで、それは元々していたことでしかありません。

署名が言うのは「このバイト列は自分の審査を通った」であって、いつ通ったかではありません。前回の署名は、いまも
同じだけそれを支えています。
